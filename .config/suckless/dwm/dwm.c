...
enum { SchemeNorm, SchemeSel, SchemeHov, SchemeHid, SchemeStatus }; /* color schemes */
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		/* 2px right padding */
		else if (ev->x > selmon->ww - stsw - getsystraywidth() + lrpad - 2) {
			x = selmon->ww - stsw + lrpad - 2 - getsystraywidth();
			int len, i;
			click = ClkStatusText;
			#if INVERSED
	...
	if (inc > 0) {
		if (selmon->sel)
			for (c = selmon->sel->next;
					c && (!ISVISIBLE(c, selmon) || (!hid && HIDDEN(c)));
					c = c->next);
		if (!c)
		...
		if (selmon->sel) {
			for (i = selmon->cl->clients; i != selmon->sel; i = i->next)
				if (ISVISIBLE(i, selmon) && !(!hid && HIDDEN(i)))
					c = i;
		} else
			c = selmon->cl->clients;
		if (!c)
...
motionnotify(XEvent *e)
{
	...
	do
		x += TEXTW(tags[i]);
	while (ev->x >= x && ++i < LENGTH(tags));
	if (i < LENGTH(tags) || ev->x < x + TEXTW(selmon->ltsymbol) || ev->x > selmon->ww - stsw + lrpad - 2) {
		if (selmon->hov) {
			if (selmon->hov != selmon->sel)
				XSetWindowBorder(dpy, selmon->hov->win, scheme[SchemeNorm][ColBorder].pixel);
			else
...
resizeclient(Client *c, int x, int y, int w, int h)
{
 ...
 /* ((nexttiled(c->mon->clients) == c && !nexttiled(c->next))
	...
	if (/* !selmon->pertag->drawwithgaps[selmon->pertag->curtag] &&  *//* this is the noborderfloatingfix patch, slightly modified so that it will work if, and only if, gaps are disabled. */
			(((nexttiled(c->mon->cl->clients, selmon) == c && !nexttiled(c->next, selmon)) /* these two first lines are the only ones changed. if you are manually patching and have noborder installed already, just change these lines; or conversely, just remove this section if the noborder patch is not desired;) */
			|| &monocle == c->mon->lt[c->mon->sellt]->arrange))
			&& !c->isfullscreen && !c->isfloating
			&& NULL != c->mon->lt[c->mon->sellt]->arrange) {
					c->w = wc.width += c->bw * 2;
...
tile(Monitor *m)
{
  ...
  if (m->pertag->drawwithgaps[m->pertag->curtag]) { /* draw with fullgaps logic */
    ...
    for (i = 0, my = ty = m->pertag->gappx[m->pertag->curtag], c = nexttiled(m->cl->clients, m); c; c = nexttiled(c->next, m), i++)
      if (i < m->nmaster) {
