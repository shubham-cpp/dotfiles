...
function get_icon(level, icons)
	if string.len(capture('pactl get-sink-mute @DEFAULT_SINK@ | grep yes')) > 0 then
		return icons[1]
	end
	local v = level or 0
	if v > 90 then
...
local function worker(user_args)
	...
	vol_widget.widget = wibox.widget({
		{
			...
			markup = capture('pamixer --get-volume-human'),
		},
		spacing = 4,
	...
	local update_widget = function(widget, stdout, _, _, _)
		...
		current_level = capture('pamixer --get-volume-human')
	...
	function vol_widget:toggle()
		spawn.easy_async(toggle_volume_cmd, function()
			spawn.easy_async(get_volume_cmd, function(out)
				update_widget(vol_widget.widget, out)
