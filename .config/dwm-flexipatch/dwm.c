...
detachstack(Client *c)
{
	...
	*tc = c->snext;
	...
	if (c == c->mon->sel) {
		for (t = c->mon->cl->stack; t && !ISVISIBLE(t, selmon); t = t->snext);
		c->mon->sel = t;
	}
	c->snext = NULL;
}
...
focus(Client *c)
{
	...
	if (!c || !ISVISIBLE(c, selmon))
		for (c = selmon->cl->stack; c && (!ISVISIBLE(c, selmon) || c->issticky); c = c->snext);
	#endif // STICKY_PATCH
	if (!c || !ISVISIBLE(c, selmon))
...
focusstack(const Arg *arg)
{
	...
	#if BAR_WINTITLEACTIONS_PATCH
	if (arg->i > 0) {
		for (c = selmon->sel->next; c && (!ISVISIBLE(c, selmon) || (arg->i == 1 && HIDDEN(c))); c = c->next);
		if (!c)
			for (c = selmon->cl->clients; c && (!ISVISIBLE(c, selmon) || (arg->i == 1 && HIDDEN(c))); c = c->next);
	} else {
		for (i = selmon->cl->clients; i != selmon->sel; i = i->next)
			if (ISVISIBLE(i, selmon) && !(arg->i == -1 && HIDDEN(i)))
				c = i;
		if (!c)
			for (; i; i = i->next)
				if (ISVISIBLE(i, selmon) && !(arg->i == -1 && HIDDEN(i)))
...
noborder(Client *c)
{
	...
	if (!monocle_layout && (nexttiled(c->mon->cl->clients, selmon) != c || nexttiled(c->next, selmon)))
		return 0;
...
restack(Monitor *m)
{
	...
	if (m->lt[m->sellt]->arrange) {
		...
		if (m->bar) {
			...
			for (f = m->cl->stack; f && (f->isfloating || !ISVISIBLE(f, m)); f = f->snext); // find first tiled stack client
			if (f)
				wc.sibling = f->win;
		}
		for (c = m->cl->stack; c; c = c->snext)
			if (!c->isfloating && ISVISIBLE(c, m) && c != f) {
...
zoom(const Arg *arg)
{
	...
	#if ZOOMSWAP_PATCH
	if (c == nexttiled(c->mon->clients, selmon)) {
		#if PERTAG_PATCH
		...
		if (!cprevious || cprevious != p) {
			...
			#endif // PERTAG_PATCH
			#if SWAPFOCUS_PATCH && PERTAG_PATCH
			if (!c || !(c = c->mon->pertag->prevclient[c->mon->pertag->curtag] = nexttiled(c->next, selmon)))
			#else
	...
	cold = nexttiled(c->mon->cl->clients, selmon);
	if (c != cold && !at)
		at = findbefore(c);
	#if FOCUSMASTER_RETURN_PATCH
	for (i = 0; !(selmon->tagset[selmon->seltags] & 1 << i); i++);
